  ...
  create(): void {
    ...
    for (const unit of units) {
      this.add.text(100, y, unit.name, {
        fontFamily: 'monospace', fontSize: '11px', color: '#b8a888',
      });
      ...
      const btn = this.add.text(700, y + 4, '[ REPAIR ]', {
        fontFamily: 'monospace', fontSize: '10px', color: '#4cae6e', letterSpacing: 1,
      }).setInteractive({ useHandCursor: true })
        .on('pointerdown', () => {
          unit.stats.hp = unit.stats.maxHp;
          unit.stats.heat = 0;
          btn.setText('REPAIRED').setColor('#7a6e5a').removeInteractive();
        });
